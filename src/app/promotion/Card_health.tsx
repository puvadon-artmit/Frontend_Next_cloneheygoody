...
export default function Card_health() {
    const classes = useStyles();

    const dispatch = useDispatch<any>();
    const healthData = useSelector((state: RootState) => selectpromotionData(state));
    ...
    return (
        <Container maxWidth="lg">
            ...
            <Grid container spacing={2} className={classes.cardGrid}>
                {healthData && healthData.length > 0 && healthData.slice(0, 4).map((car: any) => (
                     car.attributes.promotion_types?.data[0]?.attributes?.type_pm === "ประกันสุขภาพ" && (
